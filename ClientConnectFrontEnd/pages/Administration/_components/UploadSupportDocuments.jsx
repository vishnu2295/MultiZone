...
export default function UploadSupportDocuments({ id, values, setFieldValue }) {
  ...
  const handleFileChange = (event) => {
    ...
    uploadDocument.mutate(data, {
      onSuccess: (data) => {
        setFieldValue("supportDocument", [...(values || []), data.data.data]);
        setDocument("");
        handleClose();
      },
      onError: (error) => {
  ...
  const handleClose = () => {
    setDocument("");
    setDocument_type(""); // Reset document type as well
    setOpen(false);
  };
  ...
  return (
    <React.Fragment>
      ...
      <Dialog
        ...
        keepMounted
        onClose={handleClose}
        aria-describedby={`upload-documents-${id}`}>
