...
export { getRolesFromAccessToken } from "@/lib/roles";

export interface CognitoSessionClaims {
  /** Decoded, verified access token claims; undefined when signed out. */
...
/**
 * Reads the Cognito session from cookies. Pass the request when calling from
 * proxy.ts; call with no arguments from a route handler or server component,
 * where it reads via next/headers cookies().
 */
export async function getServerCognitoSession(
  request?: NextRequest,
): Promise<CognitoSessionClaims> {
  const cookieStore = request?.cookies ?? (await cookies());
  const verified = await readVerifiedAccessToken(cookieStore);
  return { accessToken: verified?.claims };
}

const RMA_IDS_CLAIM = "https://rma.com/claims/rma_ids";

/** rma_ids entries from the access token, normalised from PascalCase keys. */
export function getRmaIdsFromAccessToken(accessToken: Record<string, unknown> | undefined): RmaId[] {
  const raw = accessToken?.[RMA_IDS_CLAIM];
